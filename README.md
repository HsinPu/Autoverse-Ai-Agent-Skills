...
![Skills](https://img.shields.io/badge/Skills-268-7c3aed)
...
這組命令會安裝全部 268 個 Skills、237 個 Agents，並啟用不依賴專案 `AGENTS.md` 的全域主動委派。
...
|---|---:|---|---|
| Skills | **268 Skills**／16 類 | 可重複使用的工作流程、規範、工具指引與領域知識 | `skills/<name>/SKILL.md` |
...
268 個 Skills 分成 16 類。每個 package 以 `SKILL.md` 為入口，相關 references、scripts、assets 與 evals 保留在同一資料夾中。
...
| `threejs-graphics` | 47 | Three.js 架構、資產、渲染、互動、程序化圖形、驗證、效能、無障礙與 production delivery |
...
| Three.js application and graphics | [`threejs-development`](skills/threejs-development/) | 依實際需求路由到 46 個 specialists，涵蓋架構、生命週期、glTF、動畫、材質、Shader、互動、WebGL／WebGPU／TSL、R3F、WebXR、UI、遊戲、資料視覺化、地理空間、程序化視覺、效能、驗證與部署 |
...
- Three.js 相關 Skills 以 `threejs-development` 作為跨領域入口，並以 46 個 `threejs-*` specialists 涵蓋產品架構、生命週期、資產、動畫、互動、渲染平台、高階視覺、WebGPU／TSL、UI、遊戲、資料視覺化、地理空間、效能、驗證與部署。24 個圖形領域參考 [scottstts/Threejs-Awesome-Graphics-Agent-Skills](https://github.com/scottstts/Threejs-Awesome-Graphics-Agent-Skills) 的固定 MIT `SKILL.md` 範圍；GPL、授權不明的範例、資產、installer 與 vendor code 均未移植。完整 mapping 與稽核邊界記錄於 [`threejs-skill-sources.md`](docs/threejs-skill-sources.md)。
...
- Repository 與全部 237 個 Agents 採 Apache-2.0。Skills 的個別授權以各自 `SKILL.md` 與 `skills.json` 為準；目前 267 個為 Apache-2.0，`karpathy-guidelines` 保留 MIT 授權與外部 reference metadata。
