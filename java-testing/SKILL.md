---
name: java-testing
description: Java testing engineering guide for JUnit 5, Mockito, AssertJ, Spring test slices, Testcontainers, JaCoCo coverage, and deterministic test design. Use when writing, reviewing, or fixing Java or Spring tests, practicing TDD, adding regression tests, or diagnosing flaky JUnit test failures.
...
- **Spring slice**：controller 用 `@WebMvcTest` / `MockMvc`，JPA repository 用 `@DataJpaTest`，JSON serialization 用 `@JsonTest`。
